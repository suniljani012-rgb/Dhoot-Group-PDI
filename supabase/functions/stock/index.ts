...
serve(async (req) => {
  ...
  if (req.method === 'GET') {
    ...
    if (search) query = query.or(`vin.ilike.%${search}%,model.ilike.%${search}%,customer_name.ilike.%${search}%`);
  ...
  if (req.method === 'POST') {
    ...
    const items = body.vehicles || [body];
    const { data, error } = await supabaseClient.from('vehicles').upsert(items, { onConflict: 'vin' }).select();
    if (error) return new Response(JSON.stringify({ success: false, error: { message: error.message } }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
